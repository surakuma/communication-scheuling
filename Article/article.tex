...
%% 	\subfloat[Optimal schedule]{
		\begin{tikzpicture}[decoration={brace, amplitude=5},scale=0.5, every node/.style={scale=0.5}]
...
%%		
%%		\tikzstyle{legend}=[below=0.35, anchor=mid] 	
%%		\draw[->] (-0.1, -0.1) -- (0, -0.1)  node[legend,scale=0.65]{$0$} --(0.618,-0.1) node [legend,scale=0.65]{$\tfrac{1}{\phi}$} --(1.618,-0.1) node [legend,scale=0.65]{$\phi$} --(2, -0.1) node[right,scale=0.65]{$t$}; 
%%		
