...
awk '{printf("%d %0.9f %0.9f\n", int(100 + rand() * 800), $2, $NF - $2-$4);}' temp1 >temp2_${k}.txt
rm temp1
Y=$(awk 'BEGIN{max_memory=0;}{if($1>max_memory)max_memory=$1;}END{printf("%d", max_memory);}' temp2_${k}.txt)
...
awk 'BEGIN{sum_comm=0; sum_comp=0;}{sum_comm+= $2; sum_comp+= $3;}END{printf("%0.9f %0.9f %0.9f %0.9f", sum_comm,
                                                                              ...
                                                                              sum_comm+sum_comp);}' temp2_${k}.txt >temp1
