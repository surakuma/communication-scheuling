...
p<-ggplot(dftestMelted, aes(x=capacity, y=1/ratio_to_optimal, color=Heuristic))  + geom_smooth()
...
p<-inverse_ratio_to_optimal_best_func('results_hf_batch.txt')
ggsave(file="inverse_ratio_to_optimal_hf_batch-best.pdf", p, width=23.6, height=13.4)

p<-inverse_ratio_to_optimal_best_func('results_ccsd_batch.txt')
