...
\begin{figure}[!htb]
	...
	\end{tikzpicture}
	\caption{ \label{fig:staticOrderDynamiccorrectionExample} Different static order dynamic correction heuristic schedules for a task set of Table~\ref{tab:staticOrderDynamiccorrectionExample} with a memory capacity of 9. The $OMIM$ order is $BCDEA$.\vspace*{-0.5cm}}
\end{figure} 
...
\end{figure}
		...
		In this paper, we consider the problem of deciding the order of data transfers between two memory nodes such that overlap of communications and computations is maximized. With Exascale computing, applications face bottlenecks due to communications. Hence, it is extremely important to achieve the maximum  communication-computation overlap in order to exploit the full potential of the system. We show that the problem of determining the optimal order of data transfers is NP-complete. We proposed several data transfer heuristics and evaluated them on two molecular chemistry kernels, HF and CCSD. Our results show that some of our heuristics achieve significant overlap and their makespans are very close to the lower bound. We plan to evaluate our strategies on different applications coming from multiple domains. We also plan to study the behavior of our strategies in the context of overlapping CPU-GPU communications with computations. A runtime system aiming to expose different heuristics to maximize the communication-computation overlap at the developer level and automatically select the best one is currently underway.
