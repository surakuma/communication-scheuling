...
%%	\todo[inline]{Find a citation saying 3-machine flowshop problem is NP-Complete.}
	...
	$C$. 
	\todo[inline]{SKumar: How about this?  For simplicity, we assume throughout the paper that tasks do not have any intermediate memory requirement. Hence, amount of memory required by a task is equal to its communication volume.}
	\todo[inline]{Skumar2LED: Actually adding internal memory requirement will make NP-completeness proof straightforward. I started with this problem ;).}
...
%%	
	...
	\begin{proof}
		...
		\todo[inline]{SKumar: Do we need to mention here separately that memory requirement is equal to communication volume? I think no. right?}
		As \textit{Transfer time = Latency + (Comm Volume)/Bandwidth}. For simplicity, we consider  transfer time is same as communication volume (i.e., \textit{Latency = 0} and \textit{Bandwidth = 1}). Otherwise, we can adjust $C$ such that at any point in a schedule at max one $K_i$ and three $A_i$ tasks can be active.
