...
\begin{document}
	...
	The outline of the paper is the following. Section~\ref{sec:relatedWork} describes past work on the computations with limited memory and similar problems in the literature. In section~\ref{sec:theoreticalProof}, we present an algorithm to obtain the order of data transfers when there is not any memory capacity restriction. Then, we also prove that in general data transfer problem is NP-complete. In Section~\ref{sec:heuristics}, we propose several heuristics and discuss the conditions that might be favorable to them. We mainly consider three categories of heuristics: static heuristics, dynamic heuristics and static heuristics with dynamic corrections. Sections~\ref{sec:expSetting} describes our experimental setup and we evaluate our proposed strategies on two molecular chemistry kernels in Section~\ref{sec:expResults}. Our results show that static heuristics with dynamic corrections achieve good performance in most cases. We finally propose conclusions and perspectives in Section~\ref{sec:conclusion}.
...
\end{table}
	...
	\begin{center}
		\begin{tabular}{|c|c|c|c|}
			...
			D & 5 & 5 & 1\\ \hline
		\end{tabular}
		\caption{\label{tab:dynamicSelectionExample} A task set for dynamic schedules.\vspace*{-0.5cm}}
	\end{center}
	\end{table}
...
\begin{figure}[!htb]
	...
	\end{tikzpicture}
	\caption{ \label{fig:staticOrderDynamicCorrectionsExample} Different static order dynamic corrections heuristic schedules for a task set of Table~\ref{tab:staticOrderDynamicCorrectionsExample} with a memory capacity of 9. The $OMIM$ order is $BCDAE$.\vspace*{-0.5cm}}
\end{figure} 
...
\begin{center}
			...
			\vspace*{-0.9cm}\caption{HF and CCSD tasks characteristics.\vspace*{-0.5cm}}
...
\end{figure*}	
	...
	\includegraphics[scale=0.45]{./results/ratio_to_optimal_hf-best.pdf}
	\vspace*{-0.45cm}\caption{Best variants of all categories for HF.\vspace*{-0.45cm}}
...
\end{figure}
		...
		\begin{figure}[!t]
			\includegraphics[scale=0.45]{./results/ratio_to_optimal_ccsd-best.pdf}
			\vspace*{-0.45cm}\caption{Best variants of all categories for CCSD.\vspace*{-0.45cm}}
		...
		\begin{figure*}[!t]
			...
			\includegraphics[scale=0.45]{./results/ratio_to_optimal_batch-best.pdf}
			\vspace*{-0.5cm}\caption{Best variants of all categories where heuristics are applied in the batches of 100 tasks.\vspace*{-0.25cm}}
