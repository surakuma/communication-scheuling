...
\begin{document} %
	...
	\titlerunning{Performance Models for Data Transfers}
	% If the article title is too long for the running head, you can set
	% an abbreviated article title here
	%
%%	\author{}
	\author{Suraj Kumar \inst{1}\and 
		Lionel Eyraud-Dubois \inst{2}\and 
		Sriram Krishnamoorthy \inst{1}} %
	\authorrunning{Kumar and Eyraud-Dubois, et al.} % First names are abbreviated in the running
%%	% head.
%%	% If there are more than two authors, 'et al.' is used.
%%	%
	\institute{Pacific Northwest National Laboratory, Richland, Washington, USA \\
		\email{\{suraj.kumar, sriram\}@pnnl.gov}
		\and
		 Inria Bordeaux -- Sud-Ouest, Université de Bordeaux, France\\
...
%% We assume through out the article that communication time of a task is proportional to memory required by it, and follow conventional \textit{Comm Time = Latency + (Comm Volume)/Bandwidth} model.
%%	
...
%%\begin{figure}[htb]
%%	
%%	\includegraphics[scale=0.05]{./staticOrderSchedules}
...
%%\begin{figure}[htb]
%%\includegraphics[scale=0.05]{./staticOrderDynamicCorrectionsSchedules}
%%\caption{Task set and schedules where orders are precomputed based on different heuristics.}
