...
facet_wrap(~mc, labeller=label_parsed)  + theme(axis.ticks = element_blank(), axis.text.x = element_blank()) + xlab("") +
...
ggsave(file="ratio_to_optimal_hf.pdf", p, width=9.5, height=7.5)

p<-ratio_to_optimal_func('all_results_ccsd.txt')
ggsave(file="ratio_to_optimal_ccsd.pdf", p, width=9.5, height=7.5)
