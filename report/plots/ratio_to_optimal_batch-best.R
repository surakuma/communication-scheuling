...
#p<-ratio_to_optimal_best_func('results_batch_hf.txt')
#ggsave(file="ratio_to_optimal_hf_batch-best.pdf", p, width=23.6, height=13.4)
#
#p<-ratio_to_optimal_best_func('results_batch_ccsd.txt')
#ggsave(file="ratio_to_optimal_ccsd_batch-best.pdf", p, width=23.6, height=13.4)
...
dftest[,c(2:6)]<-NULL
dftest$"Best Static"<-do.call('pmin', c(dftest[,3:7]))
dftest$"Best Dynamic"<-do.call('pmin', c(dftest[,8:10]))
dftest$"Best Static Dynamic"<-do.call('pmin', c(dftest[,11:13]))
dftest<-dftest[,c(1,2,14:16)]
dftestMelted<-melt(dftest, "capacity")
names(dftestMelted)<-c("capacity", "Heuristic", "ratio_to_optimal")
return (dftestMelted)
...
p<-ggplot(dfMelted, aes(x=capacity, y=ratio_to_optimal, color=Heuristic))  + geom_smooth(method="loess") +
facet_wrap(~aname, scale="free", ncol=1) + ylab("Ratio to Optimal") + xlab("Memory Capacity") + theme (legend.position="bottom")
#ggsave(file="test.pdf", p, width=7, height=4)
ggsave(file="ratio_to_optimal_batch-best.pdf",p, width=6.5, height=10)
